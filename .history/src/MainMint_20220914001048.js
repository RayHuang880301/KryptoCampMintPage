...
const MainMint = ({ accounts, setAccounts }) => {
  ...
  const { data: signer } = useSigner();
	const { isLoading: switchLoading, pendingChainId, switchNetwork } = useSwitchNetwork();
  ...
  useEffect(() => {
    ...
    if (connectedChainId !== validChainId) {
      ...
      if(switchLoading && validChainId === pendingChainId) {
        getChainId()
        console.log('chainId2',connectedChainId)
      }
    }
  }, [connectedChainId, pendingChainId, switchLoading, switchNetwork])
